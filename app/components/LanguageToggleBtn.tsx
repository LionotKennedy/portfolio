...
const LanguageToggleBtn: React.FC = () => {
  const currentLocale = useLocale(); // ✅ depuis next-intl directement
  const router = useRouter();
  const pathname = usePathname();
  const t = useTranslations('Language');
  ...
  // Sync currentLang quand la locale change (ex: navigation directe par URL)
  useEffect(() => {
  ...
  const handleLanguageSelect = (lang: Language) => {
    ...
    // ✅ Remplacer le segment de locale dans le pathname et forcer un hard navigation
    const newPathname = pathname.replace(`/${currentLocale}`, `/${lang.code}`);
    router.push(newPathname);
    router.refresh(); // ✅ Force Next.js à recharger les Server Components avec la nouvelle locale
  };
