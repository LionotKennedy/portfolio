...
const getTypeConfig = (type: Experience['type']) => {
  switch (type) {
    case 'freelance':
      return { color: 'bg-purple-500/20 text-purple-400 border-purple-500/30', bgHover: 'hover:bg-purple-500/5' };
    case 'internship':
      return { color: 'bg-blue-500/20 text-blue-400 border-blue-500/30', bgHover: 'hover:bg-blue-500/5' };
    case 'project':
      return { color: 'bg-green-500/20 text-green-400 border-green-500/30', bgHover: 'hover:bg-green-500/5' };
    default:
    ...
    <ItemLayout
     ...
     >
      <div className="w-full h-full flex flex-col bg-gradient-to-br from-[#0F172A66] via-[#1E3A8A22] to-[#1E3A8A33] backdrop-blur-md">
        {/* Header */}
        <div 
          className="p-3 sm:p-4 border-b border-white/10 bg-gradient-to-r from-purple-500/20 via-purple-500/10 to-transparent flex-shrink-0"
          data-aos="fade-down"
        >
          <div className="flex items-center gap-3">
            <div className="p-2 rounded-lg bg-purple-500/20 text-purple-400 shadow-lg shadow-purple-500/10 border border-purple-500/20">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="text-sm sm:text-base font-bold text-foreground truncate">Expériences Professionnelles</h3>
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-2.5 sm:gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2 mb-1">
                        <h4 className="text-xs font-semibold text-foreground group-hover:text-purple-400 transition-colors truncate flex-1">{exp.title}</h4>
                        <span className={`flex-shrink-0 px-1.5 py-0.5 text-[9px] rounded border ${typeConfig.color}`}>
                          {exp.type === 'freelance' ? 'Free' : exp.type === 'internship' ? 'Stage' : 'Projet'}
                        </span>
                      </div>
                      ...
                      <div className="flex flex-wrap items-center gap-2 text-[10px] text-muted-foreground/70 mb-2">
                        <span className="flex items-center gap-1"><Calendar size={10} className="text-purple-400" />{exp.period}</span>
                        <span className="w-1 h-1 rounded-full bg-muted-foreground/40" />
                        <span className="truncate max-w-[80px] sm:max-w-[120px]">{exp.company}</span>
                      </div>
                      {exp.technologies && (
                        <div className="flex flex-wrap gap-1">
                          {exp.technologies.map((tech, techIndex) => (
                            <span key={techIndex} className="px-1.5 py-0.5 text-[9px] rounded bg-purple-500/10 text-purple-300 border border-purple-500/20">{tech}</span>
                          ))}
                        </div>
                      )}
                    </div>
        ...
        <div className="p-3 border-t border-white/10 bg-white/5 flex-shrink-0" data-aos="fade-up">
          <div className="flex items-center justify-between text-[10px] sm:text-xs">
            ...
            <span className="text-purple-400 font-medium flex items-center gap-1"><ExternalLink size={10} /> Voir plus</span>
          </div>
        </div>
      </div>
    </ItemLayout>
