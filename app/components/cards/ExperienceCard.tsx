...
// import React, { useEffect } from 'react';
// import { Briefcase, Calendar, ExternalLink, Code2, Building2, Rocket, Users } from 'lucide-react';
// import ItemLayout from '../about/ItemLayout';
...
//   { key: 'ecommerce', type: 'freelance', icon: <Rocket size={16} /> },
//   { key: 'mail', type: 'internship', icon: <Building2 size={16} /> },
//   { key: 'billing', type: 'internship', icon: <Users size={16} /> },
//   { key: 'hr', type: 'stage', icon: <Users size={16} /> },
//   { key: 'church', type: 'stage', icon: <Code2 size={16} /> },
// ];
...
//             </div>
//             <div className="flex-1 min-w-0">
//               <h3 className="text-sm sm:text-base font-bold text-foregroundS truncate">{t('title')}</h3>
...
//               const typeConfig = getTypeConfig(exp.type);
//               const technologies = t.raw(`jobs.${exp.key}.technologies`) as string[];
...
//                     <div className="flex-1 min-w-0">
//                       <div className="flex items-start justify-between gap-2 mb-1">
//                         <h4 className="text-xs font-semibold text-foregroundS group-hover:text-accent-400 transition-colors truncate flex-1">{t(`jobs.${exp.key}.title`)}</h4>
//                         <span className={`flex-shrink-0 px-1.5 py-0.5 text-[9px] rounded border ${typeConfig.color}`}>
//                           {t(`types.${exp.type}`)}
//                         </span>
//                       </div>
//                       <p className="text-[10px] text-muted-foreground/80 line-clamp-1 mb-1.5">{t(`jobs.${exp.key}.description`)}</p>
//                       <div className="flex flex-wrap items-center gap-2 text-[10px] text-muted-foreground/70 mb-2">
//                         <span className="flex items-center gap-1"><Calendar size={10} className="text-accent" />{t(`jobs.${exp.key}.period`)}</span>
//                         <span className="w-1 h-1 rounded-full bg-muted-foreground/40" />
//                         <span className="truncate max-w-[80px] sm:max-w-[120px]">{t(`jobs.${exp.key}.company`)}</span>
//                       </div>
//                       {technologies && (
//                         <div className="flex flex-wrap gap-1">
...
//               </span>
//               <span className="text-muted-foreground/60">•</span>
//               <span className="text-muted-foreground">{experienceKeys.filter(e => e.type === 'freelance').length} freelance</span>
//             </div>
//             <span className="text-accent font-medium flex items-center gap-1"><ExternalLink size={10} /> {t('seeMore')}</span>
//           </div>
//         </div>
//       </div>
//     </ItemLayout>
...
interface Experience {
  key: string;
  type: 'freelance' | 'internship' | 'project' | 'stage';
  icon: React.ReactNode;
}
      ...
      <div className="w-full h-full flex flex-col bg-gradient-to-br from-[#1E3A8A22] via-[#8791A666] to-[#1E3A8A33] backdrop-blur-md">
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-purple-500/30 scrollbar-track-transparent p-3 sm:p-4">
          <div className="space-y-2.5 sm:space-y-3">
            {experienceKeys.map((exp, index) => {
              const typeConfig = getTypeConfig(exp.type);
              // Utiliser une valeur par défaut si technologies n'existe pas
              const techKey = `jobs.${exp.key}.technologies`;
              let technologies: string[] = [];
              try {
                technologies = t.raw(techKey) as string[];
              } catch {
                technologies = [];
              }
